...
bool PairEndProcessor::processPairEnd(ReadPairPack* pack, ThreadConfig* config){
    ...
    for(int p=0;p<pack->count;p++){
        ...
        config->getPreStats1()->statRead(or1); // TODO 统计相关信息
        ...
        if(r1 != NULL && r2!=NULL) {
            if(mOptions->polyGTrim.enabled)
                PolyX::trimPolyG(r1, r2, config->getFilterResult(), mOptions->polyGTrim.minLen);//string resize
            if(mOptions->polyXTrim.enabled)
                PolyX::trimPolyX(r1, r2, config->getFilterResult(), mOptions->polyXTrim.minLen);//string resize
        }
        bool isizeEvaluated = false;
        //TODO 这个地方看样子是进行了一部分的分析工作
            ...
            if(mOptions->adapter.enabled) {
                ...
                if(!trimmed){
                    if(mOptions->adapter.hasSeqR1)
                        AdapterTrimmer::trimBySequence(r1, config->getFilterResult(), mOptions->adapter.sequence, false);//TODO 存在一定的substr 和 可以对一个函数进行simd优化
...
void PairEndProcessor::consumePack(ThreadConfig* config){
	...
	if(mOptions->interleavedInput){
    	...
    	processPairEnd(data, config); // 分析工作所在地
	}
	else{
