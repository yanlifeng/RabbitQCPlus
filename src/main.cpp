...
int main(int argc, char *argv[]) {
    ...
    cmd.add<int>("reads_to_process", 0,
                 "specify how many reads/pairs to be processed. Default 0 means process all reads.", false, 0);
    ...
    cmd.add<string>("adapter_sequence_r2", 0,
                    "the adapter for read2 (PE data only). This is used if R1/R2 are found not overlapped. If not specified, it will be the same as <adapter_sequence>",
                    false, "auto");
    ...
    cmd.add("cut_by_quality3", '3',
            "enable per read cutting by quality in tail (3'), default is disabled (WARNING: this will interfere deduplication for SE data)");
    cmd.add<int>("cut_window_size", 'W', "the size of the sliding window for sliding window trimming, default is 4",
                 false, 4);
    ...
    cmd.add<int>("unqualified_percent_limit", 'u',
                 "how many percents of bases are allowed to be unqualified (0~100). Default 40 means 40%", false, 40);
    ...
    cmd.add<int>("length_required", 'l', "reads shorter than length_required will be discarded, default is 15.", false,
                 15);
    cmd.add<int>("length_limit", 0, "reads longer than length_limit will be discarded, default 0 means no limitation.",
                 false, 0);
    ...
    cmd.add<int>("umi_skip", 0,
                 "if the UMI is in read1/read2, rabbit_qc can skip several bases following UMI, default is 0", false,
                 0);
    ...
    cmd.add<string>("report_title", 'R', "should be quoted with \' or \", default is \"rabbitQC report\"", false,
                    "RabbitQC report");
    ...
    if (cmd.exist("cut_window_size") && !opt.qualityCut.enabled5 && !opt.qualityCut.enabled3) {
        cerr
                << "WARNING: you've specified sliding window size (-W/--cut_window_size), but you haven't enabled per read cutting by quality for 5'(-5/--cut_by_quality5) or 3' (-3/--cut_by_quality3), so quality cutting is ignored "
                << endl << endl;
    } else if (cmd.exist("cut_mean_quality") && !opt.qualityCut.enabled5 && !opt.qualityCut.enabled3) {
        cerr
                << "WARNING: you've specified sliding window mean quality requirement (-M/--cut_mean_quality), but you haven't enabled per read cutting by quality for 5'(-5/--cut_by_quality5) or 3' (-3/--cut_by_quality3), so quality cutting is ignored "
                << endl << endl;
    }
    ...
    if (cmd.exist("split") && cmd.exist("split_by_lines")) {
        error_exit(
                "You cannot set both splitting by file number (--split) and splitting by file lines (--split_by_lines), please choose either.");
    }
    if (cmd.exist("split")) {
