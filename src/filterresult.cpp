...
FilterResult *FilterResult::merge(vector<FilterResult *> &list) {
    ...
    for (int i = 0; i < list.size(); i++) {
        ...
        // merge adapter stats
#ifdef UseUnorder
        unordered_map<string, long>::iterator iter;
#else
...
#ifdef UseUnorder
void FilterResult::outputAdaptersJson(ofstream &ofs, unordered_map<string, long> &adapterCounts) {
    unordered_map<string, long>::iterator iter;
...
void FilterResult::reportHtml(ofstream &ofs, long totalReads, long totalBases) {
    ...
    if (mOptions->correction.enabled) {
        HtmlReporter::outputRow(ofs, "reads corrected:", HtmlReporter::formatNumber(mCorrectedReads) + " (" +
                                                         to_string(mCorrectedReads * 100.0 / total) + "%)");
        HtmlReporter::outputRow(ofs, "bases corrected:", HtmlReporter::formatNumber(getTotalCorrectedBases()) + " (" +
...
#ifdef UseUnorder
void FilterResult::outputAdaptersHtml(ofstream &ofs, unordered_map<string, long> &adapterCounts,
                                      long totalBases) {
    ...
    if (frac < 0.01) {
        ofs << "<div class='sub_section_tips'>The input has little adapter percentage (~" << to_string(frac * 100.0)
    ...
    ofs << "<table class='summary_table'>\n";
    ofs << "<tr><td class='adapter_col' style='font-size:14px;color:#ffffff;background:#556699'>" << "Sequence"
        << "</td><td class='col2' style='font-size:14px;color:#ffffff;background:#556699'>" << "Occurrences"
        << "</td></tr>\n";
    ...
    for (iter = adapterCounts.begin(); iter != adapterCounts.end(); iter++) {
        ...
        ofs << "<tr><td class='adapter_col'>" << iter->first << "</td><td class='col2'>" << iter->second
            << "</td></tr>\n";
